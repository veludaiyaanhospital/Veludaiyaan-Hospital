...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 text-left text-sm font-semibold text-slate-700 transition-all hover:text-slate-900 [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
